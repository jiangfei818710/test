...
//	$GLOBALS["sPathFileName"] = $GLOBALS["sContentPath"].$GLOBALS["sSaveFileName"];
	$GLOBALS["sPathFileName"] = 'http://pic.buynow.com.cn/contentpic/'.$GLOBALS["datesCreateDir"]."/".$GLOBALS["sSaveFileName"];
...
function DoCreateNewDir(){
	...
	$GLOBALS["datesCreateDir"] = $sCreateDir;
...
function  InitUpload(){
	...
	global $sAllowExt, $nAllowSize, $sUploadDir, $nUploadObject, $nAutoDir, $sBaseUrl, $sContentPath, $datesCreateDir;
...
function ReplaceRemoteUrl($sHTML, $sExt){
	...
	for ($i=0; $i< count($a_RemoteUrl); $i++) {
		...
		if (SaveRemoteFile($SaveFileName, $a_RemoteUrl[$i])) {
			...
			$s_Content = str_replace($a_RemoteUrl[$i], 'http://pic.buynow.com.cn/contentpic/'.$GLOBALS["datesCreateDir"]."/".$SaveFileName, $s_Content);
		}
	}
